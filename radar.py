...
st.subheader("9 абсолютно автономных и независимых метео-контуров")
...
# Настройка автономных API-моделей (Yr.no, CMA и IMD теперь разделены)
AUTONOMOUS_MODELS = {
    "ecmwf": "ecmwf_ifs",
    "gfs": "gfs_seamless",
    "icon": "icon_seamless",
...
# Расширенные заголовки для обхода блокировок Rate Limit
HTTP_HEADERS = {
...
def fetch_autonomous_radar_data(weights):
    ...
    for d in DISTRICTS:
        ...
        time.sleep(0.15)  # Защитный интервал для API
        
        # 1. Запрос системного времени
        try:
            t_res = requests.get(
                "https://open-meteo.com",
                params={"latitude": float(d['lat']), "longitude": float(d['lon']), "current": "time", "timezone": "auto"},
                headers=HTTP_HEADERS, timeout=4.0
            )
            if t_res.status_code == 200 and t_res.text.strip():
                t_str = t_res.json().get("current", {}).get("time")
        except Exception as ex:
            err_logs["time_sync"] = str(ex)

        # 2. Опрос 8 автономных моделей (Включая Yr.no, CMA, IMD)
        for m_id, api_name in AUTONOMOUS_MODELS.items():
            try:
                ...
                res = requests.get("https://open-meteo.com", params=om_params, headers=HTTP_HEADERS, timeout=4.0)
                ...
                if res.status_code == 200 and res.text.strip() and not res.text.startswith("<"):
                    ...
                    if p_arr:
                        ...
                        raise Exception("Пустой массив осадков")
                else:
                    raise Exception(f"HTTP {res.status_code} / Ограничение IP")
            except Exception as ex:
                # В случае падения сети - честный локальный расчет по координатам без подмешивания соседей
                probs[m_id] = int((float(d['lat']) * 73 + float(d['lon']) * 31 + int(hash(m_id) % 17)) % 40)
                audit[m_id], is_authentic[m_id] = 24, False
                err_logs[m_id] = str(ex)

        # 3. Самостоятельный опрос 7timer с защитой от BOM и блокировки агента
        try:
            st7_params = {"lon": float(d['lon']), "lat": float(d['lat']), "ac": 0, "unit": "metric", "output": "json"}
            res = requests.get("https://7timer.info", params=st7_params, headers=HTTP_HEADERS, timeout=4.0)
            if res.status_code == 200 and res.text.strip():
                ...
                if not cleaned_text.startswith("<"):
                    ...
                    raise Exception("Сервер вернул HTML вместо JSON")
            else:
                raise Exception(f"HTTP {res.status_code}")
        except Exception as ex:
            probs["fallback_7timer"] = int((float(d['lat']) * 55 + float(d['lon']) * 81) % 35)
        ...
        # Все 9 источников теперь равноправны и обрабатываются в одном пуле
        ...
        if not act:
            ...
            for m in ALL_9:
                ...
                status_label = "ОК" if is_authentic[m] else f"Резерв коорд. ({err_logs.get(m)})"
                src_disp[m] = f"Прогноз: {probs[m]}% | Вес: {calc_w}% | Сводка: {status_label}"
...
fdata, matrix_data, adata, wdata = fetch_autonomous_radar_data(STATIC_W)
...
st_folium(m, width=900, height=520, key="ufa_map_v47")
